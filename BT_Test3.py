...
def internal_udp_receiver():
    ...
    MAX_TTL = 5
    ttl_L, ttl_C, ttl_R = 0, 0, 0
    last_L, last_C, last_R = 999.0, 999.0, 999.0
    ...
    while True:
        try:
            ...
            for obj in objects:
                # 1. Y좌표로 박스 높이 계산 ('y_min', 'y_max' 대응)
                ...
                # 4. 차선 분류 및 최단 거리 갱신 (해상도 1280 기준)
                if x_center < 500: 
                    if dist < cur_dist_L: cur_dist_L = dist       # [좌측 차선]
                elif x_center > 780: 
                    if dist < cur_dist_R: cur_dist_R = dist       # [우측 차선]
                else: 
                    if dist < cur_dist_C: cur_dist_C = dist       # [내 차선(중앙)]
            ...
            # 5. 긴급 제동(AEB) 판단: '내 차선'에 있는 차가 m 이내일 때만 작동
            if last_C < 13.0: 
                aeb_triggered = True
                aeb_flag = "1"
            else:
            ...
            if bt_serial.is_open:
                bt_serial.write(f"RADAR:{last_L:.1f}:{last_C:.1f}:{last_R:.1f}:{aeb_flag}\n".encode('utf-8'))
                
            # 터미널에서 실시간 데이터 흐름
            def fmt(d): return "없음" if d > 100 else f"{d:.1f}m"
            print(f" 레이더 -> 좌:{fmt(last_L)} | 중앙:{fmt(last_C)} | 우:{fmt(last_R)}")
...
try:
    while True:
        ...
        if aeb_triggered:
            ...
            IPC_SendPacketWithIPCHeader(sndfile, 1, 0, 0x104, parse_hex_data('00')) # 속도 0
            IPC_SendPacketWithIPCHeader(sndfile, 1, 0, 0x102, parse_hex_data('01')) # 브레이크등 ON
            sndfile.flush()
            ...
            if received_data:
                ...
                if len(parts) == 2:
                    try:
                        ...
                        if canID == 0x104 and int(sndDataHex, 16) > 0:
                            IPC_SendPacketWithIPCHeader(sndfile, 1, 0, 0x102, parse_hex_data('02'))
